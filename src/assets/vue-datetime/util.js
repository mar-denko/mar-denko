...
export function timeHoursComponentIsDisabled(
  min,
  max,
  hour,
  currentMinute,
  ...
  if (disabledTimes && disabledTimes.length > 0) {
    ...
    for (const timeRange of disabledTimes) {
      ...
      if (
        (hour === startHour && startMinute === 0) ||
        (hour === endHour && endMinute === 59) ||
  ...
  return (min !== null && hour < min) || (max !== null && hour > max);
}

export function timeMinutesComponentIsDisabled(
  min,
  max,
  minute,
  currentHour,
  ...
  return (min !== null && minute < min) || (max !== null && minute > max);
}
